  ...
  describe("Plugins Iterator - Ensure correctness #" .. strategy, function()
    ...
    it("ensure no cross-contamination", function()
      ...
      for i = 0, 10 do
        ...
        -- The plugin for service+1 is not executed
        assert.request(r).has_no_header("correlation-id-added-service" .. i + 1)
        -- The plugin for _THIS_ service is executed
        assert.request(r).has_header("correlation-id-added-service" .. i)
        -- The plugin for the service-1 is not executed
        assert.request(r).has_no_header("correlation-id-added-service" .. i - 1)

        -- The plugin for service+1 is not executed
        assert.response(r).has_no_header("response-transformed-" .. i + 1)
        -- The plugin for _THIS_ service is executed
